...
public class EnemyController : MonoBehaviour
{
    ...
    void Update()
    {
        if(player != null)
        {
            ...
            {
                Vector3 direction = (player.position - transform.position).normalized;      //두 벡터를 빼고 Normalized 하면 방향 값을 알려줌
                rb.MovePosition(transform.position + direction * speed * Time.deltaTime);   //플레이어를 향해서 설정한 speed 속도로 이동
            }

            //포탑 회전
            Vector3 targetDirection = (player.position - enemyPivot.transform.position).normalized;     //두 백터를 빼고 Normalized 하면 방향 값을 알려줌
